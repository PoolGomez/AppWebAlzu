...
import { z } from "zod";
import { formSchemaCreateRoom } from "./FormCreateRoom.form";
import {
  Form,
  FormControl,
  FormDescription,
...
export function FormCreateRoom({ companyId }: { companyId: string }) {
  const [columns, setColumns] = useState(6);
  const [rows, setRows] = useState(6);
  const [isPending, startTransition] = useTransition();
  const router = useRouter();

  const form = useForm<z.infer<typeof formSchemaCreateRoom>>({
    resolver: zodResolver(formSchemaCreateRoom),
    defaultValues: {
      name: "",
      rows: 6,
      columns: 6,
  ...
  function onSubmit(values: z.infer<typeof formSchemaCreateRoom>) {
